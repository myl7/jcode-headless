...
impl Drop for IsolatedSessionEnv {
    fn drop(&mut self) {
...
/// NOT owned by the cancelling connection (no local task handle: post-restart
...
fn cancel_aborts_detached_streaming_turn_with_stale_stop_signal() -> anyhow::Result<()> {
    ...
    let _env = IsolatedSessionEnv::new();
