...
fn credential_failure_wave_detected_for_recent_auth_failed_workers() {
    ...
    assert!(message.contains("`jcode auth import claude`"));
}
...
#[test]
fn credential_fix_hint_maps_provider_names() {
    assert_eq!(
        super::credential_fix_hint(Some("anthropic")),
        "`jcode auth import claude`"
    );
    assert_eq!(
        super::credential_fix_hint(Some("OpenAI")),
        "`jcode auth import openai`"
    );
    assert_eq!(
        super::credential_fix_hint(Some("copilot")),
        "replace the mounted credential for `copilot`"
    );
    assert_eq!(
        super::credential_fix_hint(None),
        "replace the mounted credential for `<provider>`"
    );
}
